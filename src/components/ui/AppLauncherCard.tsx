...
const AppLauncherCard: React.FC<Props> = ({ app }) => {
  ...
  return (
    <button
      ...
      className={`relative group flex flex-col p-4 rounded-xl border text-left transition-all duration-200 overflow-hidden w-full
        ${isComingSoon
          ? 'border-border bg-card/30 opacity-40 cursor-not-allowed'
          : 'border-border bg-card hover:border-border-bright hover:-translate-y-0.5 cursor-pointer'
        }`}
    >
      {/* Hover color tint */}
      {!isComingSoon && (
      ...
      <div className="flex items-start justify-between mb-3 relative z-10">
        <div
          className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0 border border-border"
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0 mt-0.5">
          ...
          {isComingSoon && <Lock size={11} className="text-muted" />}
          {app.type === 'iframe' && (
      ...
      <div className="relative z-10">
        <p className={`text-sm font-semibold leading-none mb-1.5 transition-colors
          ${isComingSoon ? 'text-muted' : 'text-bright group-hover:text-white'}`}
        >
          {app.name}
        </p>
